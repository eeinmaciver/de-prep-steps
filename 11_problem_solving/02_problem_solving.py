...
def counter_spy(people):
    ...
    for ppl in people:
        ppl = ppl.lower()
        #if 's' not in ppl and 'p' not in ppl and 'y' not in ppl:
        if not any(letter in ppl for letter in 'spy'):
            not_a_spy.append(ppl.title())
        else:
            pass

    return sorted(not_a_spy)
...
def convert_time_string(sample_string):
    ...
    hour = int(sample_string[:2])
    minute = sample_string[3:]
    ...
    twelve_hour_time = hour + ':' + minute

    return twelve_hour_time


#        sample = sample_string[:2] + sample_string[4:]
#
 #       if int(sample) >= 1300:
  #          int_sample = sample - 1200
   #         return str(int_sample[:1] + ':' + int_sample[2:])
    #    else:
