...
def largest_number(number):
    ...
    big_number = sorted(number_list, reverse=True)
    big_string = "".join(big_number)
...
def generate_matrix(number):
    ...
    for outer_numb in range(1, number + 1):
        empty.append([])
        for inner_numb in range(1, number + 1):
            empty[-1].append(None)
...
def find_wrong_way_fruit(orchard):
    ...
    fruit_count = Counter(orchard)
    most_common_fruit = fruit_count.most_common(1)
    regular_fruit = most_common_fruit[0][0]
    
    if len(orchard) < 3:
        return 0
    else:
        for position, fruit in enumerate(orchard):
            if fruit != regular_fruit:
                return position
   ...
   # for i in range(len(orchard)):
    #     for j in range(i + 1, len(orchard)):
...
def dna_pairs(dna_string):
    ...
    dna_list = list(dna_string.upper())

    match_list = []

    for dna in dna_list:
        if dna == 'G':
            match_list.append('GC')
        elif dna == 'C':
            match_list.append('CG')
        elif dna == 'T':
            match_list.append('TA')
        elif dna == 'A':
...
def tally_hashtags_and_mentions(tweet):
    ...
    mention_hashtag_dict = {}

    mention_hashtag_dict['hashtags'] = hashtag_count
    mention_hashtag_dict['mentions'] = mention_count

    return mention_hashtag_dict
