...
def capitalise_first_letter(string_thing):
    return string_thing.title()
...
def is_in_the_20th_century(x):

    if x >= 1901 and x <= 2000:
        return True
    else:
        return False
...
def get_char_code(asc_str):

    x = ord(asc_str)

    return f"The ASCII code for {asc_str} is {x}"
...
def check_battery_level(battery):
    ...
    elif battery > 5 and battery <= 99:
        return f"Battery level: {battery}%"
    else:
        return f"Warning - battery level low: {battery}%, please charge your device"
...
def collect_strings(string_list):
    return [item for item in string_list if isinstance(item, str)]
