...
def increment_even_numbers(number_list):
    ...
    even_list = [number + 1 if number % 2 == 0 else number for number in number_list ]
...
def find_average_games(games):
    ...
    average_games = {game[0]:game[1] for game in games if game[1] > 25 and game[1] < 75}
...
def get_unique_departments(employees):
    unique_departments = {employee.get("department") for employee in employees}
